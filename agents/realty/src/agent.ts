...
import { REALTY_SYSTEM_PROMPT } from './prompts';
import { REALTY_TOOLS, RealtyTools } from './tools';
...
export class RealtyAgent {
  ...
  private tools: RealtyTools;
  private conversationHistory: Map<string, AgentMessage[]> = new Map();

  constructor(config: Partial<AgentConfig> = {}) {
    this.config = {
      agentId: 'realty-agent',
      name: 'Realty Expert Agent',
      model: 'claude-3-5-sonnet-20241022',
      maxTokens: 4096,
      temperature: 0.7,
      vectorCollection: 'realty-knowledge',
      kafkaTopics: {
        consume: ['agent-messages-realty-agent', 'property-updates', 'lead-compliance'],
    ...
    this.tools = new RealtyTools();
  }

  async initialize(): Promise<void> {
    logger.info('Initializing Realty Agent');
    await this.coordinator.connect();
    await this.coordinator.subscribe(this.config.kafkaTopics.consume);
    logger.info('Realty Agent initialized');
  }
  ...
  async query(userMessage: string, sessionId?: string): Promise<string> {
    ...
    let response = await this.claudeClient.sendMessage({
      systemPrompt: REALTY_SYSTEM_PROMPT,
      messages,
      tools: REALTY_TOOLS,
    });
